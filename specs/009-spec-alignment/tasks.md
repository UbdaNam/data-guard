...
- [x] T001 Create the canonical subscriptions registry scaffold at `docs/governance/subscriptions_registry.yaml`.
...
- [x] T002 [P] Add a registry loader/helper in `contracts/registry_loader.py`.
...
- [x] T003 [P] Add canonical artifact path constants in `contracts/artifact_paths.py`.
...
- [x] T004 Populate the minimum required producer-consumer interfaces in `docs/governance/subscriptions_registry.yaml`.
...
- [x] T005 [P] Extend `contracts/registry_loader.py` with schema validation and traversal helpers.
...
- [x] T006 [P] Add baseline store helpers in `contracts/baseline_store.py`.
...
- [x] T007 [US1] Wire the registry loader into `contracts/attributor.py` and `contracts/schema_analyzer.py`.
...
- [x] T008 [US1] Expose registry-driven topology outputs in `contracts/attributor.py`.
...
- [x] T009 [US2] Add numeric baseline write support for governed numeric fields in `contracts/generator.py`.
...
- [x] T010 [US2] Inject explicit 0.0–1.0 confidence constraints in `contracts/generator.py`.
...
- [x] T011 [US2] Extract downstream consumers from the latest Week 4 lineage snapshot in `contracts/generator.py`.
...
- [x] T012 [US2] Add optional OpenRouter-based ambiguous-field annotation and deterministic fallback in `contracts/generator.py`.
...
- [x] T013 [US2] Add the numeric baseline loader and mean/stddev drift calculation in `contracts/runner.py`.
...
- [x] T014 [US2] Implement WARN > 2σ and FAIL > 3σ logic plus explicit confidence range checks in `contracts/runner.py`.
...
- [x] T015 [US2] Add `--mode` semantics for `AUDIT`, `WARN`, and `ENFORCE` in `contracts/runner.py`.
...
- [x] T016 [US3] Integrate registry-assisted subscriber resolution into `contracts/attributor.py`.
...
- [x] T017 [US3] Implement the exact attribution confidence formula and five-candidate cap in `contracts/attributor.py`.
...
- [x] T018 [US3] Enrich `blast_radius` in `contracts/attributor.py` with the required downstream impact fields.
...
- [x] T019 [US3] Implement the explicit CRITICAL classifier rule in `contracts/schema_analyzer.py`.
...
- [x] T020 [US3] Add per-consumer failure-mode analysis in `contracts/schema_analyzer.py`.
...
- [x] T021 [US3] Add rollback and baseline re-establishment requirements to migration impact rendering in `contracts/schema_analyzer.py`.
...
- [x] T022 [US4] Implement the embedding baseline writer/loader and cosine-distance drift detection in `contracts/ai_extensions.py`.
...
- [x] T023 [US4] Add governed prompt input JSON Schema validation and quarantine routing in `contracts/ai_extensions.py`.
...
- [x] T024 [US4] Add structured LLM output violation-rate tracking and WARN logging in `contracts/ai_extensions.py`.
...
- [x] T025 [US4] Keep all AI extension flows orchestrated through `contracts/ai_extensions.py` with OpenRouter-only environment-driven fallback behavior.
...
- [x] T026 [US4] Update the report health score calculation and explicit score rendering in `contracts/report_generator.py`.
...
- [x] T027 [US4] Add precise action extraction and prioritization in `contracts/report_generator.py`.
...
- [x] T028 [US4] Preserve deterministic report rendering with optional OpenRouter narrative fallback in `contracts/report_generator.py`.
...
- [x] T029 [US4] Update `.env.example`, `README.md`, and the runbooks for the new registry, validation modes, AI completion, and report behavior.
...
- [x] T030 Perform a cross-feature alignment pass across `contracts/*.py`, `README.md`, `.env.example`, and the runbooks.
